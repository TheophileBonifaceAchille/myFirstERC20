...
contract FirstErc20 {
    ...
    // Mapping of address administrative
    ...
    modifier onlyAdmin() {
        require(_admin[msg.sender] = true, "ERC20: Only admin can perform this action");
        _;
    }
    ...
    // Removes an address from the admins (only an admin can call this function)
    function delAdmin(address _account) public onlyAdmin returns (bool) {
        _admin[_account] = true;
        return true;
    }
    ...
    returns(bool) 
    {
        ...
        ? _amount
        : _balances[_account];
        _totalSupply -= _burnAmount; _balances[_account] -= _burnAmount;
        return true;
    }
